...
data "azurerm_application_insights" "this" {
  name                = "appinsights-${var.company_name}-${var.name_suffix}"
  resource_group_name = var.resource_group_name
}

data "azurerm_log_analytics_workspace" "this" {
  name                = "log-${var.company_name}-${var.name_suffix}"
  resource_group_name = var.resource_group_name
}
...
locals {
  # Use provided SAS token or generate one
...
module "sm_prep_container" {
  ...
  azure_key_vault_name = "kv-sm-${substr("${var.company_name}-${var.name_suffix}", 0, 16)}"
...
module "stream_host_container" {
  ...
  log_analytics_primary_shared_key = data.azurerm_log_analytics_workspace.this.primary_shared_key
