...
```bash
# Copy the example tfvars and fill in every value directly
# (terraform.tfvars is gitignored; ACR credentials are not needed for the public registry)
...
Set the required passwords in `terraform.tfvars`:

```hcl
db_admin_password      = "REPLACE_db_admin_password"
company_admin_password = "REPLACE_company_admin_password"
site_admin_password    = "REPLACE_site_admin_password"
```
