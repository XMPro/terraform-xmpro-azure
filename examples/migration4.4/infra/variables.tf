...
variable "enable_stream_connector" {
  description = "Enable Stream Connector infrastructure (MQTT broker on Azure Container Apps)"
  type        = bool
  default     = false
}

variable "use_existing_mqtt_broker" {
  description = "Use an existing external MQTT broker instead of deploying one"
  type        = bool
  default     = false
}
...
variable "mqtt_enable_tls" {
  description = "Enable TLS encryption for the MQTT broker (port 8883). Defaults to true."
  type        = bool
  default     = true
}