# Application layer for 4.4 -> 4.6 in-place migration. Apply after infra/.
# All migration inputs are explicit tfvars. URL rewrite is a separate layer
# (migration/).
...
locals {
  ...
  # ============================================================================
  # EFFECTIVE VALUES - tfvar override wins, else KV lookup, else safe default
  # ============================================================================

  # Existing Product IDs / Keys - operator-supplied tfvars, lowercased to
  # match what SM emits in OIDC audience claims (4.6 OIDC is case-sensitive).
  effective_sm_product_id  = var.use_existing_database ? lower(var.existing_sm_product_id) : ""
  effective_ad_product_id  = var.use_existing_database ? lower(var.existing_ad_product_id) : ""
  effective_ds_product_id  = var.use_existing_database ? lower(var.existing_ds_product_id) : ""
  effective_ai_product_id  = var.use_existing_database && var.enable_ai ? lower(var.existing_ai_product_id) : ""
  effective_ad_product_key = var.use_existing_database ? var.existing_ad_product_key : ""
  effective_ds_product_key = var.use_existing_database ? var.existing_ds_product_key : ""
  effective_ai_product_key = var.use_existing_database && var.enable_ai ? var.existing_ai_product_key : ""

  # SMTP - direct from tfvars with safe defaults when null.
  effective_smtp_server               = var.smtp_server != null ? var.smtp_server : ""
  effective_smtp_from_address         = var.smtp_from_address != null ? var.smtp_from_address : "noreply@xmpro.com"
  effective_smtp_username             = var.smtp_username != null ? var.smtp_username : ""
  effective_smtp_password             = var.smtp_password != null ? var.smtp_password : ""
  effective_smtp_port                 = var.smtp_port != null ? var.smtp_port : 587
  effective_smtp_enable_ssl           = var.smtp_enable_ssl != null ? var.smtp_enable_ssl : true
  effective_enable_email_notification = var.enable_email_notification != null ? var.enable_email_notification : false

  # AD encryption key - tfvar wins, else empty (_app auto-generates).
  effective_ad_encryption_key = var.ad_encryption_key != null ? var.ad_encryption_key : ""

  # SQL Server FQDN - required tfvar when use_existing_database = true.
  effective_existing_sql_server_fqdn = var.existing_sql_server_fqdn != null ? var.existing_sql_server_fqdn : ""

  # Redis / AutoScale - direct from tfvars with defaults.
  effective_redis_connection_string = var.redis_connection_string != null ? var.redis_connection_string : ""
  effective_enable_auto_scale       = var.enable_auto_scale != null ? var.enable_auto_scale : false
}
...
module "applications" {
  ...
  existing_sql_server_fqdn = local.effective_existing_sql_server_fqdn

  # Operator-supplied tfvars (lowercased for product IDs in locals above)
  existing_sm_product_id  = local.effective_sm_product_id
  existing_ad_product_id  = local.effective_ad_product_id
  existing_ds_product_id  = local.effective_ds_product_id
  existing_ai_product_id  = local.effective_ai_product_id
  existing_ad_product_key = local.effective_ad_product_key
  existing_ds_product_key = local.effective_ds_product_key
  existing_ai_product_key = local.effective_ai_product_key
  ...
  enable_auto_scale         = local.effective_enable_auto_scale
  redis_connection_string   = local.effective_redis_connection_string
  enable_email_notification = local.effective_enable_email_notification
  ...
  smtp_server       = local.effective_smtp_server
  smtp_from_address = local.effective_smtp_from_address
  smtp_username     = local.effective_smtp_username
  smtp_password     = local.effective_smtp_password
  smtp_port         = local.effective_smtp_port
  smtp_enable_ssl   = local.effective_smtp_enable_ssl
