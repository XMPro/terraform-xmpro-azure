...
resource "azurerm_linux_web_app" "ad_app" {
  ...
  # creating the app. Adding module.ad_secrets serves two purposes:
...
resource "azapi_update_resource" "ad_min_tls_cipher_suite" {
  count       = var.min_tls_cipher_suite != null ? 1 : 0
  type        = "Microsoft.Web/sites/config@2023-12-01"
  resource_id = "${azurerm_linux_web_app.ad_app.id}/config/web"
