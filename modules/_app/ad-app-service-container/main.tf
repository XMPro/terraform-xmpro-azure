...
locals {
  ...
  identity_name = substr("id-ad-${var.name_suffix}", 0, 128)
}
...
# Add access policy for AD identity to access Key Vault
resource "azurerm_key_vault_access_policy" "ad_identity_policy" {
  key_vault_id = data.azurerm_key_vault.ad_key_vault.id
  tenant_id    = azurerm_user_assigned_identity.ad_identity.tenant_id
  object_id    = azurerm_user_assigned_identity.ad_identity.principal_id
...
resource "azurerm_linux_web_app" "ad_app" {
  ...
  depends_on = [azurerm_key_vault_access_policy.ad_identity_policy]
