...
locals {
  ...
  identity_name = substr("id-ai-${var.name_suffix}", 0, 128)
...
# Add access policy for AI identity to access Key Vault
resource "azurerm_key_vault_access_policy" "ai_identity_policy" {
  key_vault_id = data.azurerm_key_vault.ai_key_vault.id
  tenant_id    = azurerm_user_assigned_identity.ai_identity.tenant_id
...
resource "azurerm_linux_web_app" "ai_app" {
  ...
  depends_on = [azurerm_key_vault_access_policy.ai_identity_policy]
