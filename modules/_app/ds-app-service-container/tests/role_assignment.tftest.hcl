...
variables {
  ...
  db_connection_string           = "Server=tcp:sql.test,1433;Initial Catalog=DS;"
  ...
  ds_product_id                  = "00000000-0000-0000-0000-000000000000"
  ds_product_key                 = "00000000-0000-0000-0000-000000000000"
  ds_key_vault_name              = "kv-test"
  ds_service_plan_name           = "plan-test"
...
run "role_assignment_pins_principal_type" {
  ...
  assert {
    condition     = azurerm_role_assignment.ds_identity_secrets[0].principal_type == "ServicePrincipal"
    error_message = "azurerm_role_assignment.ds_identity_secrets must set principal_type = \"ServicePrincipal\" (omitting it causes intermittent PrincipalNotFound on fresh identities)"
  }
}
